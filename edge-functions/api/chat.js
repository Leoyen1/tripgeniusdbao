// 为了避免混淆，建议直接使用 functions/api/chat.js 进行部署。
// 如果您必须使用 Edge Function (Workers) 模式，请使用以下代码：

/**
 * Tencent Cloud EdgeOne Edge Function (Standard Worker Syntax)
 */
  ...
  async fetch(request, env) {
    ...
    if (request.method === "GET") {
        return new Response(JSON.stringify({
            status: "ok",
            message: "TripGenius Edge Function is active",
            env_check: { has_key: !!env.API_KEY }
    ...
    if (request.method === "POST") {
        ...
        if (!API_KEY || !MODEL_ID) {
            return new Response(JSON.stringify({ 
              error: "Configuration Error: API_KEY or DOUBAO_MODEL_ID missing." 
            }), {
              status: 500,
              headers: { "Content-Type": "application/json", ...corsHeaders }
            });
        ...
        try {
            ...
            if (!upstreamResponse.ok) {
                ...
                return new Response(JSON.stringify({ error: `AI Provider Error: ${upstreamResponse.status} ${errText}` }), {
    ...
    // 4. 其他方法返回 400 (Bad Request)，避免 405 歧义
    return new Response("Bad Request: Method Not Supported", { status: 400, headers: corsHeaders });
  }
};